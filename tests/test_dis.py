...
class DfTestCase(unittest.TestCase):
    ...
    def test_diversity_of_df(self):
        ...
        self.assertAlmostEqual(
            0.98 * 0.02 + 0.01 * 0.99 + 0.01 * 0.99,
            s_diversity[3], places=10
        )
...
class IntegrationTestCase(unittest.TestCase):
    ...
    def test_di(self):
        ...
        for region in df_di.index:
            self.assertAlmostEqual(self.integration[region], df_di.loc[region]['integration'], places=10)
...
class IntegrationOverNonNumericTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.integration_region_over_neighborhood = {
            'X': (
                    ...
                    (90.0 / 480.0) * (2.0 / 3.0) +  # X102
                    (100.0 / 480.0) * 0.0 +  # X201
                    (200.0 / 480.0) * 0.5  # X202
            ),
            'Y': 0.0,
            'Z': 0.03 * (2.0 / 3.0)
        }
    ...
    def test_di_region_over_subregion(self):
        ...
        for region in df_di.index:
            self.assertAlmostEqual(self.integration_region_over_subregion[region], df_di.loc[region]['integration'], places=10)
            self.assertAlmostEqual(self.regional_diversity[region], df_di.loc[region]['diversity'], places=10)
